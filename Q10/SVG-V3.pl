...
$j = $j + 1;
my @data = split /[\t\n]+/;
	if ($data[3] eq '+' && $data[2] eq "chr8" && $data[4] > 1960000 && $data[5] < 2150000){ #select part of genome fall within ruler range as pilot study
...
# main for plot graph
for my $key (sort {$genelist{$a}->get_txStart <=> $genelist{$b}->get_txStart} keys %genelist){ #get gene information from list of UCSCgene object
...
for (my $i =0; $i <= $row; $i++) {
	...
	print $g->get_chrom."\t".$g->get_genename."\t".$g -> get_txStart. "\t".$g -> get_txEnd ."\t".$g -> get_cdsStart. "\t".$g -> get_cdsEnd."\n";
...
sub plot_gene {
	...
	plot_utr ($left_margin_width, $g, $tmp_y,$scale);
}
...
# use for loop to write all exon start and end location 
		...
		if ($x2 == 0){$x2=1;} # Zero correction as 1 to restore a mark
		print "exon: $exon_beg[$i] \t $exon_end[$i] \n";		
		print "exon: $x1 \t $x2 \n";
...
sub plot_intron {
	...
	my @exon_beg = split /,/, $g->get_exonStarts;
 	my @exon_end = split /,/, $g->get_exonEnds;
# use for loop to write all exon start and end location then calculate intron location
		...
		print "intron: $x1 \t ",$x2-$x1," \n";
		$svg->line('x1',$x1,'y1',$tmp_y+10,'x2',$x2,'y2',$tmp_y+10,'stroke','green','stroke-width',1);
		for (my $a = 0; $a <= int(($x2-$x1)/8);$a++){
			$svg->line('x1',$x1+$a*8,'y1',$tmp_y+10,'x2',$x1+$a*7.5,'y2',$tmp_y + 14,'stroke','green','stroke-width',1);
			$svg->line('x1',$x1+$a*8,'y1',$tmp_y+10,'x2',$x1+$a*7.5,'y2',$tmp_y + 6,'stroke','green','stroke-width',1);}	
		}
}
...
sub plot_utr{
	...
	my $tmp_y = shift;
	my $scale = shift;	
	my $cdsStartU = $g->get_cdsStart;
	my $cdsEndU = $g->get_cdsEnd;
	my $geneStart = $g->get_txStart;
	my $geneEnd = $g->get_txEnd;
	
	my $x1 = int(($geneStart - $genomic_position_start)/$scale) + $left_margin_width;
	my $x2 = int(($cdsStartU - $geneStart)/$scale);
		if ($x2 == 0 && $geneStart != $cdsStartU){ $x2 = 1;} # Zero correction as 1 to restore a mark
		if ($cdsStartU == $cdsEndU){ $x2 = 0;} # remove UTR is CDS start equal CDS end
		$svg->rect('x',$x1,'y',$tmp_y,'width',$x2,'height',20,'fill','white','fill-opacity',1,'stroke','green','stroke-width',1);

	my $x3 = int(($cdsEndU - $genomic_position_start)/$scale) + $left_margin_width;
	my $x4 = int(($geneEnd - $cdsEndU)/$scale);
		if ($x4 == 0 && $cdsEndU != $geneEnd){ $x2 = 1;} # Zero correction as 1 to restore a mark
		if ($cdsStartU == $cdsEndU){ $x4 = 0;} # remove UTR is CDS start equal CDS end		
		$svg->rect('x',$x3,'y',$tmp_y,'width',$x4,'height',20,'fill','white','fill-opacity',1,'stroke','green','stroke-width',1);

	print "utr: $geneStart \t $cdsStartU \t $geneEnd \t $cdsEndU \n";	
	print "utr: $x1 \t $x2 \t $x3 \t $x4 \n";
}
...
sub plot_ruler {
	...
	my $x1 = $left_margin_width; my $x2 = ($left_margin_width) + int($length/$scale) ; 
		$svg->line('x1',$x1,'y1',$y-12,'x2',$x2,'y2',$y-12,'stroke','black','stroke-width',1);
	for (my $i=0; $i <= $ticN; $i++) {
		my $tmp_x= int($x1 + $i * ($tick_unit/$scale));
		my $mark = int($genomic_position_start + $i * $tick_unit)/1000;
		$svg->line('x1', $tmp_x, 'y1', $y-12, 'x2', $tmp_x, 'y2', $y-20, 'stroke','black','stroke-width',1);
		if ($i == $ticN) {
